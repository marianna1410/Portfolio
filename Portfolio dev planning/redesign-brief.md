...
## 11. Відомі дрібні недоліки — на етап polish/QA

### 11.1. Hero-анімація: мікро-миготіння в Opera (низький пріоритет)
**Симптом:** у момент reveal Hero-анімації (skeleton → контент) пігулка `OPEN TO WORK` дає **мілісекундне** миготіння **лише в Opera** (майже непомітно). У Chrome/Chromium — чисто. Раніше так само мигали H1 і хедер — це вже виправлено (див. нижче).

**Причина:** перемалювання/рекомпозит GPU-шарів у момент reveal. Крапка-статус пігулки використовує `filter: blur` (окремий шар, що перерастеризовується). Opera на цьому кадрі показує шар порожнім на мить; Chromium — ні.

**Що вже зроблено (частково допомогло):**
- Клас стану анімації перенесено з `.hero` на `.hero__cards` — H1/хедер/заголовкова група більше не перемальовуються при reveal (H1/хедер мигати перестали).
- Крапці пігулки (`.tag__dot`) додано стабільний шар (`transform: translateZ(0)` + `isolation: isolate`, `translateZ` на blur-псевдоелементі).

**Що спробувати в polish:** замінити в крапці-статусі `filter: blur` на `box-shadow`-світіння (виглядає майже так само, але не створює шар, що мигає) — і перевірити в Opera. Те саме за потреби для конекторних крапок Hero (теж `filter: blur`). Не критично: Chrome/Safari/Firefox рендерять чисто.
